...
> **O aviso de `ADR-002` continua valendo:** *serverless* (Lambda, Cloud Functions, Vercel Functions)
...
O Redis existe na arquitetura para **três** coisas (`DOC-022 §4.3`): *rate limiting* distribuído,
...
O arquivo [`render.yaml`](../render.yaml) na raiz é um *blueprint*: o Render lê ele e cria os dois
...
> erro no console fala de *mixed content*, não de WebSocket — o que manda a investigação para o lado
> errado.
