...
| Campo | Valor |
|---|---|
| **Produto** | AetherTable — plataforma web *sandbox* para Magic: The Gathering, **multiformato**, com Commander/EDH como formato-vitrine |
...
### Trilha 1 — Produto e negócio *(por que construímos)*
...
### Trilha 2 — Requisitos e comportamento *(o que o sistema faz)*
...
### Trilha 3 — Arquitetura *(como o sistema é montado)*
...
### Trilha 4 — Contratos e engine *(as interfaces exatas)*
...
### Trilha 5 — Frontend e experiência *(como aparece na tela)*
...
### Trilha 6 — Segurança, qualidade e operação *(como não cair)*
...
### Trilha 7 — Administração e Monetização *(Backoffice)*
...
| **Colyseus** | Framework Node.js de salas multiplayer com sincronização automática de estado por *delta*. |
...
| **SFU** | *Selective Forwarding Unit*: servidor que recebe uma trilha de áudio e a redistribui. |
| **VAD** | *Voice Activity Detection*: detecção de quem está falando. |
...
2. **`runbook_de_incidentes.md`** — procedimento de plantão: sala travada, SFU fora, fila de Scryfall.
3. **`politica_de_moderacao.md`** — fluxo de report/bloqueio de `RF03` ponta a ponta.
4. **`plano_de_internacionalizacao.md`** — i18n de UI e de nomes de carta (a Scryfall suporta `lang`).
5. **Diagramas renderizados** — hoje os diagramas são ASCII/Mermaid; avaliar exportar SVG versionado.
