...
**Por quê:** renderizar centenas de cartas com *drag-and-drop* e zoom em `<div>` trava o navegador —
...
**Por quê:** leve, sem *boilerplate*, sem *Provider hell*, e com seletores que evitam re-render
...
| `gameStore` | espelho do `RoomState` alimentado pelos *patches* | **Servidor** |
...
SDK oficial. Aplica os *patches* binários no espelho local e expõe *callbacks* de mudança
...
os *payloads* de intenção via `packages/shared-types` (`ADR-007`).
...
**Por quê:** injeção de dependência, módulos, *guards* e *pipes* de validação prontos. Para um backend
...
O fator decisivo, porém, é o **`@filter`**: ele implementa informação oculta *no servidor* (`RN02`,
...
Começa em **LiveKit Cloud** para não administrar SFU; migra para *self-hosted* quando o custo
justificar.
...
| Tokens | JWT de acesso (15 min) + *refresh* rotativo em cookie `HttpOnly; Secure; SameSite=Strict` |
...
| Biblioteca | Uso |
|---|---|
| `zod` | Validação de todo *payload* HTTP e WS (`FR-11`) |
| `@nestjs/throttler` + Redis | *Rate limiting* (`NFR-04`) |
...
SQL Injection por *parameterized queries* (relevante para `DOC-051` §1.1).
...
| Uso | Criticidade |
|---|---|
| *Rate limiting* distribuído | Alta em produção |
...
| *Snapshot* de sala (futuro, `ADR-006`) | — |
...
| **PostgreSQL** | Neon, Supabase ou RDS | Backup automático e *point-in-time recovery* | Postgres em VPS (mais trabalho) |
| **Redis** | Upstash ou Redis gerenciado | Simplicidade | Redis em VPS |
| **Voz** | LiveKit Cloud | Evita administrar SFU | LiveKit *self-hosted* |

> **Aviso recorrente:** não hospedar o game server em plataforma serverless. WebSocket exige conexão
> persistente; *cold start* e limite de duração de execução quebram a partida.
...
|---|---|
| **Núcleo** (Next, React, Colyseus, Prisma) | Atualizar *minor* mensalmente; *major* só com janela dedicada e teste de carga |
| **Segurança** | Patch aplicado em até 72 h para CVE alta/crítica |
| **Ferramental** (lint, build) | Livre, desde que o CI passe |
| **Colyseus** | **Atenção especial:** mudanças de *major* alteram o formato de serialização. Frontend e backend precisam subir juntos. |
...
| Redux | Zustand | *Boilerplate* desproporcional |
