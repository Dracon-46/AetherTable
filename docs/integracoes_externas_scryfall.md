...
banco próprio de cartas: armazenamos apenas o `scryfall_id` (UUID de *printing*) e hidratamos
...
**Regra de ouro:** chamadas de *busca e resolução* (texto → UUID) passam pelo backend, para
centralizar cache e respeitar rate limit. Chamadas de *imagem* vão direto do browser para a CDN
...
| Não fazer *scraping* de imagens em massa | — | Imagens são carregadas pelo browser sob demanda, com cache. |
...
| `429` | Rate limit excedido | *Backoff* exponencial: 1 s, 2 s, 4 s, 8 s (máx. 4 tentativas). Depois disso, erro 503 para o cliente. |
...
const LINE = /^\s*(?<qty>\d+)\s*[xX]?\s+(?<name>[^([#*]+?)\s*(?:\((?<set>[A-Za-z0-9]{2,5})\)\s*(?<cn>\S+)?)?\s*(?:\[(?<set2>[A-Za-z0-9]{2,5})\])?\s*(?:\*F\*)?\s*(?:#.*)?$/;
```
...
2. **Service Worker** (`CacheStorage`) — intercepta `cards.scryfall.io`, estratégia *cache-first*,
...
- **Atribuição.** O rodapé do Deckbuilder exibe: *"Dados de cartas fornecidos pela Scryfall."*
