...
O sistema atua como *sandbox* (mesa virtual) **sem aplicação de regras do jogo**. Os jogadores
...
|---|---|---|---|---|---|---|
| **F01** | Busca integrada de cartas | Consulta a Scryfall com filtros de nome, tipo, cor, custo de mana e texto de regras. Paginação de 60 resultados com *scroll* infinito. Debounce de 300 ms no campo. | S | MVP | RF16 | API + EXT |
...
| **F17** | Verificação de *color identity* | Sinaliza cartas fora da identidade de cor do comandante. **Aviso apenas** — coerente com `RN01`. | C | V1 | RF06 | API |
| **F18** | Estatísticas do deck | Curva de mana, distribuição de tipos, contagem de terrenos, *color pips*. Calculado no cliente a partir do cache. | C | V1 | — | FE |
...
| **F23** | Zona de comando com *commander tax* | Contador de +2 cumulativo por conjuração, ajustável manualmente. | S | V1 | RF19 | GS |

### 3.2 Manipulação de cartas

| ID | Funcionalidade | Detalhe | Prio | Fase | RF | Lado |
|---|---|---|---|---|---|---|
| **F07** | Arrastar e soltar | Movimentação livre no Battlefield com coordenadas contínuas. *Lock* de arraste no servidor evita disputa entre dois jogadores pela mesma carta. Interpolação suave no cliente do oponente. | **M** | MVP | RF08, RF17 | FE + GS |
...
| **F09** | Ações de grimório | "Comprar 1" (`D`), "Comprar X", "Embaralhar" (`S`), "Olhar o topo N", "Mover para o fundo", "*Mill* X para o cemitério", "Exilar X do topo". Embaralhar e comprar resolvem no servidor. | **M** | MVP | RF18 | GS |
...
| **F26** | Marcadores tipados | `+1/+1`, `-1/-1`, *charge*, *loyalty*, genérico. Cada tipo tem cor própria no *badge*. | S | V1 | RF17 | GS |
...
| **F30** | *Untap* em massa | Um comando desvira todas as cartas próprias no Battlefield. | **M** | MVP | RF17 | GS |
