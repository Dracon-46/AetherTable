...
**Conceito.** Uma plataforma web multiplayer em tempo real que simula uma mesa virtual (*sandbox*)
...
**Público-alvo.** Jogadores de Commander que desejam testar decks (*playtest*) antes de comprar,
...
| **Customização** | Escolha de artes específicas (*printings*) e idiomas | `F04` |
...
|---|---|
| **Battlefield (mesa)** | Movimentação livre (*drag-and-drop*) de cartas |
...
| **Command Zone** | Área destacada com contador de taxa do comandante (*commander tax*) |

**Interações com as cartas** — hoje `F08`, `F10`, `F24`, `F25`:

- **Virar/desvirar (tap/untap):** rotação de 90°, com atalho de teclado.
- **Marcadores:** adicionar e remover (+1/+1, -1/-1, *charge* etc.).
- **Tokens e cópias:** criar fichas em branco ou puxar tokens oficiais da API.
- **Virar para baixo:** cartas de Morph/Disguise (*face-down*).
...
- *Ping* na mesa (clicar para destacar um ponto/carta para os outros verem).
...
> Como é um sistema *sandbox*, o segredo do sucesso **não é a lógica de jogo, mas a sincronização de
...
| **Motor da mesa (crucial)** | **React Konva (Canvas API)** | Usar HTML/DOM (`div`) para centenas de cartas com *drag-and-drop* e zoom travaria o navegador. O Canvas usa aceleração por GPU, garantindo 60 FPS mesmo com a mesa lotada |
...
| Camada | Escolha | Justificativa original |
|---|---|---|
| Servidor de jogo | **Node.js + Colyseus** | *"Fuja do Socket.io puro."* O Colyseus foi feito para jogos baseados em salas e tem *State Synchronization* automática — envia apenas o que mudou na mesa, poupando muita banda |
...
- *Drag-and-drop* (mover de `Hand` para `Battlefield`).
...
| *(não previsto aqui)* | **V2**: espectadores, lobby público, i18n, brackets |
...
| `counters` como `map<string,int>` | Um único inteiro não distingue `+1/+1` de *charge* ou *loyalty* |
