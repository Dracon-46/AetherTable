...
_Oathbreaker_, _Tiny Leaders_ ou _Pauper EDH_ custa minutos, não sprints.
...
especial — o _feitiço-assinatura_, uma mágica instantânea ou feitiço que também fica na zona de comando
...
function validarDeck(deck: Deck, preset: FormatPreset): Aviso[] {
  ...
  if (preset.deck.singleton) {
    /* aponta duplicatas não-básicas */
  }
  if (preset.deck.rarityCeiling) {
    /* aponta raridade acima do teto */
  }
  if (preset.deck.maxManaValue) {
    /* aponta CMV acima do limite */
  }
  if (preset.deck.legalityKey) {
    /* usa legalities da Scryfall: banned / restricted */
  }
  if (preset.commander?.enforceColorIdentity) {
    /* aponta fora da identidade */
  }
