...
Devido à complexidade do estado em tempo real (WebSocket) e da renderização gráfica (Canvas), a
estratégia de testes se divide em camadas bem definidas, **priorizando a estabilidade das salas de
jogo**.
...
```
                  ...
                  ╱────╲    lentos, alto valor
                 ╱      ╲
                ╱ Integr.╲  Integração               ~80 testes
               ╱  ────────╲ API + salas WS
              ╱            ╲
             ╱   Unitários  ╲  Unitários             ~400 testes
            ╱────────────────╲ rápidos, isolados
```

### 2.1 Testes unitários

**Ferramenta:** Jest (com `ts-jest`).
...
média diluída do repositório inteiro.

### 2.2 Testes de integração

**Ferramentas:** Supertest (backend), `@colyseus/testing` (salas), React Testing Library (frontend).
...
**Ferramenta:** Playwright (preferido a Cypress pelo suporte nativo a múltiplos contextos de
navegador — indispensável para testar 4 jogadores).

**Caminho feliz principal (`G5`):**

1. Login na plataforma.
2. Navegação até o Deckbuilder.
3. Importação de uma decklist de 100 cartas.
4. Criação de uma sala de jogo (host).
5. Verificação de que o Canvas renderizou as cartas iniciais.
6. Compra de carta e arraste para o Battlefield.
7. Verificação da entrada no log.

**Cenário multiplayer (o mais valioso do conjunto):**

```
contextoA = browser.newContext()   // Jogador A
contextoB = browser.newContext()   // Jogador B

A: cria sala → captura link de convite
B: entra pelo link → escolhe deck
A: compra carta e joga no Battlefield
B: assert → vê a carta na posição correta
B: assert → o tráfego WS de B NÃO contém scryfallId da mão de A   ← G1
A: rola D20
B: assert → vê o mesmo resultado no log
B: rede cortada → assert overlay "reconectando" em ambos
B: rede restaurada → assert estado idêntico ao anterior
```

**Cobertura de casos de uso** conforme `DOC-012` §2.

### 2.4 Teste de auditoria de informação oculta (`G1`)

Este é o **único teste que sozinho bloqueia o release**. Merece implementação dedicada.

```ts
// e2e/hidden-info.spec.ts
test('oponente nunca recebe identidade de carta em zona oculta', async ({ browser }) => {
  const frames: string[] = [];
  ...
  const capturado = frames.join('|');

  // nenhum scryfallId das cartas na mão de A pode aparecer
  for (const id of maoDoJogadorA) {
    expect(capturado).not.toContain(id);
  }
  // nenhum nome de carta oculta pode aparecer
  for (const nome of nomesDaMaoDeA) {
    expect(capturado).not.toContain(nome);
  }
});
```

Variações obrigatórias: mão, grimório, carta face para baixo no Battlefield e — na V2 — **espectador
conectado**.

---

## 3. Testes de carga e stress (WebSockets)

Este é o teste mais crítico de infraestrutura do projeto.

**Ferramenta:** k6 (preferido) ou Artillery.io, ambos com suporte a WebSocket.
...
## 4. Testes de performance de render (QA visual)

Parcialmente manual, porque a percepção visual é o critério.
...
- [ ] Imagem quebrada exibe moldura com o nome.
- [ ] MDFC renderiza e permite virar a face.
- [ ] Carta face para baixo mostra o verso para o oponente.
- [ ] `aria-live` anuncia mudanças do log.
- [ ] `prefers-reduced-motion` desliga animações não essenciais.
- [ ] Painel invisível não bloqueia clique no Canvas.
