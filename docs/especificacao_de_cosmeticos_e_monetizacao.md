...
O fundo das cartas (quando estão *face-down* na mesa ou no topo do grimório).
...
A imagem de fundo do *Battlefield* individual do jogador. Os oponentes verão o playmat renderizado na região pertencente àquele jogador.
...
- **Dimensões:** As imagens devem ser *seamless* (teláveis) ou cobrir uma proporção *ultrawide*, pesando no máximo 500KB (WEBP).
...
- **Formatos Suportados:** 
  1. **CSS Puro:** Efeitos de *glow*, gradientes rotativos e neon. Altamente recomendados pela performance nula no CPU.
...
Um pequeno *badge* (insígnia) ao lado do nome do usuário nas mensagens do chat.
...
2. O AetherTable recebe *webhooks* do Patreon informando alterações no *Tier* (nível de apoio) do usuário.
...
- O jogador então *equipa* um item do seu inventário (`UserPreference`).
