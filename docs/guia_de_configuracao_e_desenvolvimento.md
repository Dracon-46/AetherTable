...
|---|---|---|
| `frontend` | **3000** | Next.js — interface e mesa em Canvas |
...
> "token inválido" no *handshake* — e é o erro nº 1 de quem monta o ambiente pela primeira vez.
...
1. Abra http://localhost:3000 e entre com `jogador1@teste.com` / `teste1234`.
...
- [ ] Intenção nova tem *schema* Zod e verificação de autorização?
- [ ] Nenhum I/O dentro de *handler* de intenção?
...
| Cartas sem imagem | `SCRYFALL_USER_AGENT` vazio ou *rate limit* | Preencher o `User-Agent`; aguardar |
...
| Porta em uso | Outro processo na 3000/3333/2567 | `npx kill-port 3000` ou ajustar a porta |
| Tipos não resolvem | `shared-types` não compilado | `pnpm build --filter shared-types` |
| FPS baixo em dev | *Source maps* e HMR pesam | Medir performance sempre com `pnpm build && pnpm start` |
| Erro de CORS | Origem não permitida | Incluir `http://localhost:3000` em `CORS_ORIGINS` |
