...
| **Técnica**    | Tabletop Simulator exige compra do software (~R$ 40), GPU dedicada e _setup_ manual de mods.                                                                | Médio                     |
...
O **AetherTable** é uma plataforma web _sandbox_ gratuita, acessível direto pelo navegador.
O sistema **não atua como juiz de regras** (_rules engine_); ele fornece:

- Tabuleiro virtual com zonas de jogo, coordenadas livres e manipulação por _drag-and-drop_.
...
| Um cliente mobile nativo                          | O jogo é web; mobile atende Lobby/Deckbuilder, e a mesa exige _landscape_.                                                                        |
| Um substituto do papel                            | É ferramenta de _playtest_ e jogo casual remoto.                                                                                                  |
...
| --------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------- |
| **O Testador** ("vale a pena comprar?") | Joga presencial, quer validar deck antes de gastar | Não quer comprar R$ 800 em cartas para descobrir que o deck é ruim | _Playtest_ solo e em grupo com o deck completo, sem custo |
...
Tornar-se a plataforma **não-oficial padrão global** para _playtest_ e jogos casuais remotos de
...
| **H3 — Ecossistema** | 18 meses+  | API pública de replay, integração com Moxfield/Archidekt, ligas comunitárias, _overlay_ para streamers. |
...
| -------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------- |
| Performance do Canvas com 300+ sprites | Alta        | `NFR-01`; _culling_ de fora de tela, texturas compartilhadas, `small` como padrão. Ver `DOC-040`. |
| Escalonamento horizontal do Colyseus   | Alta        | `@colyseus/redis-presence` + _driver_ Redis + LB com afinidade. Ver `DOC-053`.                    |
...
O projeto opera **estritamente** sob a _Fan Content Policy_ da Wizards of the Coast:
...
- É **obrigatório** exibir o _disclaimer_ de Fan Content em todas as páginas.
...
| **Qualidade de áudio premium** | Salas de apoiador com _bitrate_ de áudio maior                                                   | ✅ Serviço de infraestrutura, não conteúdo de jogo |
...
### MVP — meses 1–3 · _"Uma partida acontece"_
...
### V1 — meses 4–6 · _"É bom de usar"_
...
### V2 — meses 7–12 · _"Escala e comunidade"_

- Modo espectador.
- Lobby público com _matchmaking_ casual.
- Brackets de torneio comunitário (sem premiação).
- Otimização de banda e _culling_ agressivo.
- Internacionalização (pt-BR, en, es) incluindo nomes de carta via `lang` da Scryfall.
- Cosméticos de apoiador.

### Pós-V2 — candidatos

Replay de partida, _overlay_ para streamers, importação direta de Moxfield/Archidekt por URL,
...
2. A Fan Content Policy da WotC não muda de forma a proibir _sandboxes_ web.
...
2. Sem _rules engine_, em qualquer versão.
3. Sem monetização de gameplay, em qualquer versão.
4. A mesa exige tela em _landscape_ (tablet ou desktop).
...
| Frustração por falta de motor de regras  | Média         | Médio     | Comunicar o modelo _sandbox_ de forma explícita no _onboarding_; log de ações claro      |
| Custo de voz explodir                    | Média         | Alto      | Limites por sala; degradar _bitrate_ antes de negar serviço; SFU próprio se justificar   |
